...
DEFAULT_NUTRITION = {
    'calories': 150, 'protein_g': 3.0, 'fat_g': 5.0,
    'carbs_g': 20.0, 'sugar_g': 8.0, 'fiber_g': 1.0,
    'sodium_mg': 100, 'gi_value': 55, 'gi_category': 'medium'
}

def lookup_food(food_name: str) -> dict:
    """
    Returns nutrition dict for given food name.
    Uses keyword match first, then fuzzy match fallback.
    """
    if not food_name:
    ...
    mask = _foods_df['food_name_lower'].str.contains(query, na=False)
    matches = _foods_df[mask]
    if not matches.empty:
        row = matches.iloc[0]
        return _row_to_dict(row)

    # 2. Word-level match — check if any word in query matches
    SKIP_WORDS = {'salty', 'sweet', 'spicy', 'hot', 'cold', 'fresh',
    ...
    for word in words:
      mask = _foods_df['food_name_lower'].str.contains(word, na=False)
      matches = _foods_df[mask]
      if not matches.empty:
        row = matches.iloc[0]
        return _row_to_dict(row)
    ...
    # 4. Nothing found — return defaults
...
if __name__ == "__main__":
    ...
    for food in tests:
        ...
        print(f"{food:<15} → sugar={result['sugar_g']}g  "
              f"sodium={result['sodium_mg']}mg  gi={result['gi_value']}  "
              f"found={result.get('found', True)}")
